---
name: local-executor
description: 本地模型落实执行者规程：把单个契约子任务的代码生成强制交给本地模型（经 llama-multimodel-mcp 的 chat/complete），执行者自身只组装提示词、应用输出、跑验证。供无子智能体机制的宿主（Codex 等）内嵌派发，也是子智能体定义的正文来源。
when_to_use: >-
  编排者（hybrid-orchestrate）派发契约执行任务包清单上的落实子任务（含系统级操作：
  装依赖、改配置、起服务、接插件）时使用本规程；或用户要求"用本地模型执行这个子任务"时。
  输入：profile_id + 最小任务包。一次只改一个文件。
---
