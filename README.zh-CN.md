...
- **按模型强弱分工。** 云端模型负责规划、接线、审计；业务代码每一行都由本地模型写。
  路由是**硬策略，不是提示词礼仪**——契约清单上的任务机械地派给本地执行者，
  "任务太复杂/系统级"不是有效的跳过理由。
- **让本地模型保持可靠。** 本地 27-35B 模型是合格的**填空执行者**，不是**自驱工程师**。
  所以每次派发都是有边界的任务包（≤5 个文件片段、一次一文件、立即验证），
  前置自信度闸门，后置强模型对照契约审计——本地模型从不给自己的作业打分。
...
```
                    ┌────────────────────────────┐
                    │  orchestrator (云端模型)      │  规划 · 派发 · 审计 · 验收
                    └─────────────┬──────────────┘
              契约任务包，          │ 派发（profile_id + 最小任务包）
              硬路由策略            │ ——编排者绝不亲自写契约代码
                                  ▼
                    ┌────────────────────────────┐
                    │ local-executor 子智能体       │  自信度闸门 ·
                    │                             │  组装提示词 · 应用输出 · 验证
                    └─────────────┬──────────────┘
                                  ▼
                    ┌────────────────────────────┐
...
| 执行者安全 | 构造提示词前的**自信度闸门**——关键信息缺失时报 `NEEDS_CONTEXT` 而不是硬写；重复失败后的**咨询-记录回路**把云端诊断折进最后一次重试，把有效修法记进 `.guild/lessons.md`，同一个坑不踩两次 |
...
> 前置：Python 3.10+、任一支持的 agent 工具，以及一个在跑的本地后端——或者没有也行：
