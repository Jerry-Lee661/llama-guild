...
同一错误签名**连续失败第 2 次**时进入 CONSULT：动用你自身的云端侧推理产出结构化诊断（`[CONSULT] 错误签名 / 根因假设 / 提示词修正点 / 缺失参考`），折叠进第 3 次重试的提示词——指导归你，业务代码仍归本地模型。第 3 次仍失败才标 `LOCAL_MODEL_FAILED`。经 CONSULT 翻盘的任务，向工作区 `.guild/lessons.md` 追加一行（日期 | 任务签名 | 错误签名 | 有效做法），供后续任务包检索复用。
